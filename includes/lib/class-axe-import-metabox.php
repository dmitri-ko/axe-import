...
if ( ! class_exists( 'Axe_Import_Metabox' ) ) :
	...
	class Axe_Import_Metabox {
		...
		public function __construct( $meta_box ) {
			...
			$this->_Local_images = ( isset( $meta_box['local_images'] ) ) ? $meta_box['local_images'] : false;
			...
			if ( isset( $meta_box['use_with_theme'] ) ) {
				if ( $meta_box['use_with_theme'] === true ) {
					...
					$this->SelfPath = plugins_url( 'axe-import' ) . '/assets';
				} else {
			...
			if ( isset( $meta_box['nonce_name'] ) ) {
				$this->nonce['name'] = $meta_box['nonce_name'];
			}

			if ( isset( $meta_box['nonce_action'] ) ) {
				$this->nonce['action'] = $meta_box['nonce_action'];
			}
		...
		public function show() {
			...
			if( isset( $this->nonce['name'] ) ) {
				if( isset( $this->nonce['action'] ) ) {
					wp_nonce_field( $this->nonce['action'], $this->nonce['name'] );
				} else {
					wp_nonce_field( basename( __FILE__ ), $this->nonce['name'] );
				}
			}
        ...
        <span class="re-control-clear"></span></div>';
			...
			echo '<span class="dashicons dashicons-insert" id="add-' . $field['id'] . '"></span></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px; min-height: 50px}
		 ...
		 * @access public
		 */
		public function show_field_posts( $field, $meta, $is_reapeter = false ) {
			global $post;

			if ( ! is_array( $meta ) ) {
				$meta = (array) $meta;
			}
			$this->show_field_begin( $field, $meta, ! $is_reapeter );
				...
				foreach ( $posts as $p ) {
					...
					if ( in_array( $id, $meta, true ) ) {
						if ( 'axe_image' === $options['post_type'] ) {
							...
							$img_meta = get_post_meta( $p->ID, '_headerID' );
							if ( $img_meta && is_array( $img_meta ) && isset( $img_meta[0] ) ) {
								$image_post_id = Axe_Import_Admin_API::get_post_id_by_meta_key_and_value( '_id', $img_meta[0] );
							} else {
								$image_post_id = $p->ID;
							}
							$img_meta = get_post_meta( $image_post_id, '_imageID' );
							if ( $img_meta && is_array( $img_meta ) && isset( $img_meta[0] ) ) {
		...
		public function save( $post_id ) {
			...
			foreach ( $this->_fields as $field ) {
				...
				// Validate meta value
				if ( class_exists( 'Axe_Import_Metabox_Validate' ) && method_exists( 'Axe_Import_Metabox_Validate', $field['validate_func'] ) ) {
					$new = call_user_func( array( 'Axe_Import_Metabox_Validate', $field['validate_func'] ), $new );
				}
